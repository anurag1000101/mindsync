...
rootdir = "/nlsasfs/home/nltm-st/sujitk/temp/eeg2text/datasets/zuco/task_nr/Matlab files"
...
for file in tqdm(os.listdir(rootdir)):
    if file.endswith(task+".mat"):
        ...
        if subject != 'YMH':
            ...
            f = h5py.File(os.path.join(rootdir,file),'r')
            ...
            for idx in range(len(rawData)):
                ...
                sent_obj['raw_eeg'] = np.squeeze(f[rawData[idx][0]][()])

                print(f"Size of raw_eeg: {sent_obj['raw_eeg'].shape}")
                # print(sent_obj)
                sent_obj['word'] = []
